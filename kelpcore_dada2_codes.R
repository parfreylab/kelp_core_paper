...
####Environment Setup####
theme_set(theme_bw())
setwd("~/desktop/corekelp_project/")

####File Path Setup####
#this is so dada2 can quickly iterate through all the R1 and R2 files in your read set
path <- "/Users/jungsoopark/desktop/corekelp_project/raw_FASTAQ/" # CHANGE ME to the directory containing the fastq files
list.files(path)
...
####Primer Removal####
####identify primers#### the default: 515F and 806R Primers
...
filterAndTrim(fnFs, fnFs.filtN, fnRs, fnRs.filtN, trimLeft = 12, maxN = 0, multithread = TRUE, compress = TRUE)
...
#### primer removal ####
cutadapt <- "/Users/jungsoopark/Library/Python/3.7/bin/cutadapt" # CHANGE ME to the cutadapt path on your machine
...
####dereplication#### to fix the error "Not all provided files exist."
...
b <- which(otus_rel_ab.rowsums <= 0.0005) #here is where you set your relative abundance threshold #which ASVs pass our filter for relative abundance
...
taxa <- assignTaxonomy(seqtab.nosingletons.nochim, "~/Desktop/R_project/silva/silva_nr_v132_train_set.fa.gz", multithread=TRUE)
## add species information to taxonomy data
taxa <- addSpecies(taxa, "~/Desktop/R_project/silva/silva_species_assignment_v132.fa.gz")
...
taxa[NAs,1] <- "Unidentified" #apply new label to identified indices
#colnames(taxa) <- c("Rank1", "Rank2", "Rank3", "Rank4", "Rank5", "Rank6")


####saving taxonomy data####
write.table(data.frame("row_names"=rownames(taxa),taxa),"taxonomy_assigned_table.txt", row.names=FALSE, quote=F, sep="\t")
...
taxa <- fread("taxonomy_assigned_table.txt", sep="\t", header=T, colClasses = c("row_names"="character"), data.table=FALSE)
...
write.table(data.frame("row_names"=rownames(seqtab.nosingletons.nochim),seqtab.nosingletons.nochim),"sequence_assigned_table_ASV_rename.txt", row.names=FALSE, quote=F, sep="\t")
write.table(data.frame("row_names"=rownames(taxa),taxa),"taxonomy_assigned_table_ASV_rename.txt", row.names=FALSE, quote=F, sep="\t")
...
#now write to file
write.table(data.frame("row_names"=rownames(sequence_taxonomy_table),sequence_taxonomy_table),"sequence_taxonomy_table_merged.txt", row.names=FALSE, quote=F, sep="\t")

#### hand off to PhyloSeq ####
#load sample data
rawmetadata <- read.csv(file="metadata_kelpcore_project.csv")
rawmetadata <- read.csv(file = file.path("~/desktop/corekelp_project/", "metadata_kelpcore_project.csv"), # file.path() is used for cross-platform compatibility
...
colnames(rawmetadata) <- c("sample_id", "sample_type", "time", "date", "location", "practitioner", "plate")
#record samples absent in either metadata or OTU table
notinmeta <- setdiff(row.names(seqtab.nosingletons.nochim), rawmetadata$sample_id)
notinraw <- setdiff(rawmetadata$sample_id, row.names(seqtab.nosingletons.nochim))
...
saveRDS(ps.dada2_join, "kelpcore_phyloseq_format.RDS")
...
# at this point I recommend saving your full unfiltered dataset in phyloseq format as a .RDS, so that you can pick up the analysis from this point easily if you decide to change your filtering criteria later on
#saveRDS(ps.dada2_join, "calvert16S.full_dataset.phyloseq_format.RDS")
#ps.dada2_join <- readRDS("calvert16S.full_dataset.phyloseq_format.RDS") #to start from here, later, run this command
...
ps.dada2_join <- ps.dada2_join %>%
  subset_taxa(Order != "Chloroplast") %>% #you can chain as many of these subset_taxa calls as you like into this single command using the R pipe (%>%)
...
saveRDS(ps.dada2_join, "kelpcore_phyloseq.RDS")
